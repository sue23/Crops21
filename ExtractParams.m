function [Data,Data100] = ExtractParams(acqpar,AnglesUL,AnglesLL,emgBP,env,forces,CoM,Moments,Powers,Doriscenter,TRJ,markers_hands,markers_head,markers_feet)
...
for s = 1:nstride
    ...
    if ~isempty(Doriscenter)
        ...
        Doriscenter_100z= [Doriscenter_100z interp1(1:nc,Doriscenter(footstrike1(s):footstrike2(s),3),t100,'linear')'];

        foot_100x= [foot_100x interp1(1:nc,TRJ(footstrike1(s):footstrike2(s),1),t100,'linear')'];
        foot_100y= [foot_100y interp1(1:nc,TRJ(footstrike1(s):footstrike2(s),2),t100,'linear')'];
        foot_100z= [foot_100z interp1(1:nc,TRJ(footstrike1(s):footstrike2(s),3),t100,'linear')'];



        hand_100x= [hand_100x interp1(1:nc,nanmean(squeeze(markers_hands(footstrike1(s):footstrike2(s),1,:)),2),t100,'linear')'];
        hand_100y= [hand_100y interp1(1:nc,nanmean(squeeze(markers_hands(footstrike1(s):footstrike2(s),2,:)),2),t100,'linear')'];
        hand_100z= [hand_100z interp1(1:nc,nanmean(squeeze(markers_hands(footstrike1(s):footstrike2(s),3,:)),2),t100,'linear')'];

        for i = 1:size(markers_head,3)
            head_100x= [head_100x interp1(1:nc,markers_head(footstrike1(s):footstrike2(s),1,i),t100,'linear')'];
            head_100y= [head_100y interp1(1:nc,markers_head(footstrike1(s):footstrike2(s),2,i),t100,'linear')'];
            head_100z= [head_100z interp1(1:nc,markers_head(footstrike1(s):footstrike2(s),3,i),t100,'linear')'];
        end

        for i = 1:size(markers_feet,3)
            feet_100x= [feet_100x interp1(1:nc,markers_feet(footstrike1(s):footstrike2(s),1,i),t100,'linear')'];
            feet_100y= [feet_100y interp1(1:nc,markers_feet(footstrike1(s):footstrike2(s),2,i),t100,'linear')'];
            feet_100z= [feet_100z interp1(1:nc,markers_feet(footstrike1(s):footstrike2(s),3,i),t100,'linear')'];
        end

        Doriscenter_100(:,1,s)=Doriscenter_100x;
        Doriscenter_100(:,2,s)=Doriscenter_100y;
        Doriscenter_100(:,3,s)=Doriscenter_100z;

        foot_100(:,1,s)=foot_100x;
        foot_100(:,2,s)=foot_100y;
        foot_100(:,3,s)=foot_100z;

        hand_100(:,1,s)=hand_100x;
        hand_100(:,2,s)=hand_100y;
        hand_100(:,3,s)=hand_100z;

        head_100(:,:,1,s)=head_100x;
        head_100(:,:,2,s)=head_100y;
        head_100(:,:,3,s)=head_100z;

        feet_100(:,:,1,s)=feet_100x;
        feet_100(:,:,2,s)=feet_100y;
        feet_100(:,:,3,s)=feet_100z;

%         keyboard
%         figure
%         set(gcf,'pos',[2645 470 560 420])
%         plot(Doriscenter_100(:,3,s))
%         hold on
%         plot(foot_100(:,3,s))
    ...
    try
        ...
        Ankle.DorsalFlexMaxSwingPerc(s) = nan;
        Ankle.DorsalFlexMaxSwing(s) = nan;
    end
    ...
    if isempty(CoM)
        CoM_100(:,1,s)=ones(100,1)*nan;
        CoM_100(:,2,s)=ones(100,1)*nan;
        CoM_100(:,3,s)=ones(100,1)*nan;
        vel_CoM_100(:,1,s)=ones(100,1)*nan;
        vel_CoM_100(:,2,s)=ones(100,1)*nan;
        vel_CoM_100(:,3,s)=ones(100,1)*nan;
    else
...
if ~isempty(Doriscenter)
    Data100.Doriscenter_100=Doriscenter_100;
    Data100.foot_100 = foot_100;
    Data100.head_100 = head_100;
    Data100.hand_100 = hand_100;
    Data100.feet_100 = feet_100;
end
if ~isempty(AnglesUL)
