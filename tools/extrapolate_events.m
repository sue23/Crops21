...
% keyboard
if isfield(events,'events') && isfield(events.events,'general')
    if ~isempty(events.events.general.event.vframe) && length(events.events.general.event.vframe)>1
...
%     Dyl =smoothdata(Dyl,'gaussian');
%     Dyr =smoothdata(Dyr,'gaussian');
    ...
    plot(Dyl)
    hold on
    plot(l_fo,Dyl(l_fo),'r*')
    plot(l_hs,Dyl(l_hs),'rd')
    plot(Dyr)
    hold on
    plot(r_fo,Dyr(r_fo),'g*')
    plot(r_hs,Dyr(r_hs),'gd')
